...
func init() {
	...
	flag.Float64Var(&currentConfig.OlapReadPool.TimeoutSeconds, "queryserver-config-query-stream-pool-timeout", defaultConfig.OlapReadPool.TimeoutSeconds, "query server stream query pool timeout (in seconds), it is how long vttablet waits for a connection from the stream query pool. If set to 0 (default) then the overall query timeout is used instead.")
